...
function createCard() {
    ...
    let card = ""; // CAMBIO: usar let y acumular

    productos.map((producto) => {
        const nombre = producto.producto.replace(/'/g, "\\'");
        const categoria = producto.categoria ? producto.categoria.replace(/'/g, "\\'") : "Ofertas";
        ...
        card += `
            ...
            `<span class="badge bg-secondary tallas-badge m-1">${talla}</span>`
                    ...
                    <button 
                        ...
                        data-imagen="${imagen}"
                        data-precio="${precio}"
                        data-preciooferta="${precioOferta}"
    ...
    container.innerHTML = card; // Al final de map
}